...
void TextAnimator::update() {
    ...
    if (is_animating && (millis() - last_update_time >= char_interval)) {
        ...
        if (current_char_index >= total_chars) {
            ...
            int display_char_index = (current_char_index * display_chars_total) / phonetic_chars_total;
...
void TextAnimator::segmentTexts(const String& displayText, const String& phoneticText) {
    ...
    int start = 0;
    int pos = 0;
    while (pos < displayText.length() && segment_count < 10) {
        if (displayText[pos] == '\n') {
    ...
    if (start < displayText.length() && segment_count < 10) {
    ...
    start = 0;
    pos = 0;
    while (pos < phoneticText.length() && phonetic_segment_count < 10) {
        if (phoneticText[pos] == '\n') {
    ...
    if (start < phoneticText.length() && phonetic_segment_count < 10) {
